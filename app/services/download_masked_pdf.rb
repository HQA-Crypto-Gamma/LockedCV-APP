...
module LockedCV
  # Downloads a saved masked PDF attachment through LockedCV-API.
  class DownloadMaskedPdf
    ...
    def call(attachment_id:, masked_attachment_id:, auth_token:)
      @client.get_binary(
        "/attachments/#{attachment_id}/masked_attachments/#{masked_attachment_id}/download",
        auth_token:
      )
    ...
    def unavailable_error_for(error)
      ServiceUnavailableError.new("Masked PDF download API unavailable: #{ApiClient.error_details(error)}")
    end
  end
end
